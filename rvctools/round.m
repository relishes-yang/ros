...
center = [0.5 0 0.3]; % 设置圆形轨迹的圆心坐标(X,Y,Z)
radius = 0.25;         % 设置圆形轨迹的半径
theta = ( N / N(end) ) * 2*pi;  % 计算轨迹角度，范围0~2π
points = ( center + radius*[cos(theta) sin(theta) zeros(size(theta))] )';  % 生成圆形轨迹坐标点
